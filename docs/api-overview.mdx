...
description: "Endpoint-by-endpoint guide. Companion to the auto-generated reference."
---
...
```json
{
  "name": "worker", "provider": "unix_local",
...
```json
{"message":"analyze the dataset"}  → {"rpc_id":"...","status":"ok"}
```
...
POST /sessions/{id}/cancel    — best-effort `session/cancel`. Always returns {"status":"ok"}.
```
