...
@pytest.mark.asyncio
async def test_codex_authenticates_once_before_treating_auth_error_as_terminal(monkeypatch):
    client = AcpClient("http://example.invalid")
    monkeypatch.setattr(client, "handshake", AsyncMock(return_value={}))
    monkeypatch.setattr(client, "set_mode", AsyncMock(return_value=None))
    send_rpc = AsyncMock(side_effect=[
        AcpError(ACP_AUTHENTICATION_REQUIRED, "Authentication required"),
        {},
        {"sessionId": "inner-1", "configOptions": []},
    ])
    monkeypatch.setattr(client, "_send_rpc", send_rpc)

    await client.initialize("session-1", "codex")

    assert [call.args[1] for call in send_rpc.await_args_list] == [
        "session/new",
        "authenticate",
        "session/new",
    ]
