"""DockerVolumeAdapter — per-volume ops for the docker provider.

Concrete ``BaseVolumeAdapter`` for docker; delegates to the module-level
volume functions in ``docker/__init__.py``.
"""
...
class DockerVolumeAdapter(BaseVolumeAdapter):
    provider = "docker"
