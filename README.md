...
On a trusted client host, `CODEX_AUTH_JSON_FILE=~/.codex/auth.json` provides the
same cache without application code reading the file. Cache auth takes
precedence over Codex token/API-key environment variables.
...
- [Honeycomb compatibility delta](docs/honeycomb.md) — intentional differences on `honeycomb-compat`
